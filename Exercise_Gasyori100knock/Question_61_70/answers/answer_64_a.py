...
while count > 0:
    ...
    for y in range(H):
        for x in range(W):
            ...
            ## condition 2
            if 1 - out_a[y, min(x + 1, W - 1)] + 1 - out_a[max(y - 1, 0), x] + 1 - out_a[y, max(x - 1, 0)] + 1 - out_a[
                min(y + 1, H - 1), x] >= 1:
                judge += 1
            ...
            ## condition 5
            c = 0
            c += (out_a[y, min(x + 1, W - 1)] - out_a[y, min(x + 1, W - 1)] * out_a[max(y - 1, 0), min(x + 1, W - 1)] *
                  out_a[max(y - 1, 0), x])
            c += (out_a[max(y - 1, 0), x] - out_a[max(y - 1, 0), x] * out_a[max(y - 1, 0), max(x - 1, 0)] * out_a[
                y, max(x - 1, 0)])
            c += (out_a[y, max(x - 1, 0)] - out_a[y, max(x - 1, 0)] * out_a[min(y + 1, H - 1), max(x - 1, 0)] * out_a[
                min(y + 1, H - 1), x])
            c += (out_a[min(y + 1, H - 1), x] - out_a[min(y + 1, H - 1), x] * out_a[
                min(y + 1, H - 1), min(x + 1, W - 1)] * out_a[y, min(x + 1, W - 1)])
            if c == 1:
                judge += 1
            ...
            c = 0
            c += (out_a[y, min(x + 1, W - 1)] - out_a[y, min(x + 1, W - 1)] * out_a[
                max(y - 1, 0), min(x + 1, W - 1)] * 0)
